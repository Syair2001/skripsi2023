...
export default function EditSentimen({params}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/list-sentimen/${params.id}`, formDataToSend)
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`/sentimen/${params.id}`, {withCredentials: true})
  ...
  return (
    <div className='text-black flex items-center justify-center h-screen'>
      ...
      <div className='p-6 shadow-lg w-4/12 rounded-md'>
        <h1 className='text-center font-semibold'>Edit Sentimen</h1>
        <form action="" onSubmit={handleSubmit} encType='multipart/form-data'>
          ...
          <div className="mb-4 flex justify-between">
            <Link href="/list-sentimen" className="text-blue-500 hover:underline">Kembali ke List Sentimen</Link>
